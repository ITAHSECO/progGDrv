...
class FilePanel:
    ...
    def _add_file(self):
        dialog = FileDialog(self.main_window.root, title="Agregar Archivo")
        self.main_window.root.wait_window(dialog.top)
        if dialog.result:
            r = dialog.result
            file_id = db.add_file(r["name"], r["source_path"], r["drive_folder_id"])
            db.update_schedule(
                file_id, r["hour_start"], r["hour_end"], r["interval_hours"],
    ...
    def _edit_file(self):
        ...
        dialog = FileDialog(
            ...
            file_data=file_data, schedule_data=schedule,
        )
        self.main_window.root.wait_window(dialog.top)
        if dialog.result:
            r = dialog.result
...
class FileDialog:
    def __init__(self, parent, title="Dialogo", file_data=None, schedule_data=None):
        self.result = None
    ...
    def _build_form(self, file_data, schedule_data):
        ...
        ttk.Label(tab_file, text="Carpeta Drive ID:").grid(row=2, column=0, sticky=tk.W, pady=3)
        self.var_folder = tk.StringVar(value=file_data["drive_folder_id"] if file_data else "")
        ttk.Entry(tab_file, textvariable=self.var_folder, width=45).grid(row=2, column=1, pady=3)
        ...
        ttk.Label(tab_sched, text="Intervalo (horas):").grid(row=0, column=0, sticky=tk.W, pady=3)
        self.var_interval = tk.DoubleVar(value=schedule_data["interval_hours"] if schedule_data else 2.0)
        ttk.Entry(tab_sched, textvariable=self.var_interval, width=10).grid(row=0, column=1, sticky=tk.W, pady=3)

        ttk.Label(tab_sched, text="Hora inicio (0-23):").grid(row=1, column=0, sticky=tk.W, pady=3)
        self.var_hstart = tk.IntVar(value=schedule_data["hour_start"] if schedule_data else 0)
        ttk.Entry(tab_sched, textvariable=self.var_hstart, width=10).grid(row=1, column=1, sticky=tk.W, pady=3)

        ttk.Label(tab_sched, text="Hora fin (0-23):").grid(row=2, column=0, sticky=tk.W, pady=3)
        self.var_hend = tk.IntVar(value=schedule_data["hour_end"] if schedule_data else 23)
        ttk.Entry(tab_sched, textvariable=self.var_hend, width=10).grid(row=2, column=1, sticky=tk.W, pady=3)
        ...
        for i, (label, key) in enumerate(zip(day_labels, day_keys)):
            val = schedule_data[key] if schedule_data else 1
